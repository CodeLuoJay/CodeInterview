...
B = 0000 1101
-----------------
A&B = 0000 1100		// 按位与 1&1=1 其余情况均为0
A | B = 0011 1101	// 按位或 有1则为1
A ^ B = 0011 0001	// 按位异或	相同为0 不同为1
~A= 1100 0011		// 按位	0变1 1变0
```

> #### 常见笔试面试题

```
System.out.println(~33);// 33 = 100001 按位取反 => 011110 =>30
```
...
这类题目主要是将整数换算成二进制的形式然后按照每个位置一一再计算，所以又叫按位运算符

### 按位左移、按位右移、按位右移补零操作符

| 操作符 | 描述                                                         | 例子                   |
| ------ | ------------------------------------------------------------ | ---------------------- |
| <<     | 有符号按位左移运算符，正数高位补0，负数高位补1               | 11 <<2 得到44          |
| >>     | 有符号按位右移运算符，正数高位补0，负数高位补1               | -11 >>2 得到-3         |
| >>>    | 按位右移补零操作符，又叫**无符号按位右移运算符**，无论正数还是负数，高位通通补0. | -11 >>>2得到1073741821 |

在计算机中，整数都是以机器数0和1的形式存储的（也叫原码）这是计算机基础的知识。

在这基础上区分的正负数的办法，就是最高位作为符号位，0代表整数、1代表负数。

举例子，以8位二进制的机器数分别表示11和-11，它们的二进制表现形式如下

```
+11 = 00001011	//1011是最基本的表示,其余是符号位
-11 = 11110101	//0101是最基本的表示，其余是符号位
```

在这基础上，要涉及到计算机运算问题，那么就由衍生出反码和补码，整数的反码和补码都是一样

负数的反码是符号位保持不变，其余位取反，负数的补码是在其反码的基础上+1，-11的原码、反码、补码如下
